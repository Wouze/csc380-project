...
def build(parent):
    ...
    def do_delete():
        ...
        try:
            cn = get_connection()
            cur = cn.cursor()
            payment_id = int(id_var.get())
            cur.execute("SELECT COUNT(*) FROM invoice WHERE payment_id = %s", (payment_id,))
            invoices = cur.fetchone()[0]
            cn.close()

            msg = f"Delete payment {payment_id}?"
            if invoices:
                msg += f"\n\nThis will also remove {invoices} linked invoice(s)."
            if not ask_yes_no(frame, "Payments", msg):
                return
