...
def build(parent):
        ...
        try:
            ...
            if search:
                if search.isdigit():
                    ...
                    cur.execute("SELECT room_id, hotel_id, room_number, floor, room_type, price_per_night, max_capacity, status FROM room WHERE room_number LIKE %s", ("%" + search + "%",))
            else:
    ...
    def do_insert():
        ...
        try:
            ...
            cur = cn.cursor()
            cur.execute(
                "INSERT INTO room (room_id, hotel_id, room_number, floor, room_type, price_per_night, max_capacity, status) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
