...
3. **Database and tables**
   - **First run of the GUI:** if MySQL is running with the defaults below, the app **creates** `hotel_management` and applies `car_rental_app/schema.sql` automatically when the `hotel` table is missing.
   ...
   Other options:
   - **phpMyAdmin** (often `http://localhost/phpmyadmin`): new database `hotel_management`, import `car_rental_app/schema.sql`.
   - **mysql** CLI (paths vary by OS):
     ...
     # "C:\xampp\mysql\bin\mysql.exe" -u root -p hotel_management < car_rental_app/schema.sql
     ```
     ...
     mysql -u root -p hotel_management < car_rental_app/schema.sql
     ```
...
From this folder (`python-hotel-project/` in the repo root, so `car_rental_app` is importable):
...
| `car_rental_app/schema.sql` | Tables, FKs, and constraints |
| `car_rental_app/schema_setup.py` | First-run: create DB if missing, apply schema if no `hotel` table |
| `car_rental_app/main.py` | Window, tabbed notebook, tab refresh on switch |
| `car_rental_app/config.py` | Environment-based DB settings |
| `car_rental_app/db.py` | `get_connection()` context manager |
| `car_rental_app/tabs/*.py` | One module per CRUD tab |

## Troubleshooting (XAMPP)

- **`Can't connect to MySQL server`** — Start MySQL in XAMPP; confirm port 3306 is not blocked or changed.
- **`ModuleNotFoundError: No module named 'mysql'`** — `pip` installed into a different Python than the one running the app. Run `python -m pip install -r requirements.txt` (same `python` you use for `python -m car_rental_app.main`), or activate your venv first, or use `conda install -c conda-forge mysql-connector-python` if you run everything under Conda.
